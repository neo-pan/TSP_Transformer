...
def pad_to_window_size_3d(hidden_state: torch.Tensor, attention_mask: torch.Tensor,
                       one_sided_window_size: int, pad_token_id: int):
    '''A helper function to pad tokens and mask to work with the sliding_chunks implementation of Longformer selfattention.
    Input:
        hidden_state = torch.Tensor(bsz x seqlen x embed_dim): ids of wordpieces
        attention_mask = torch.Tensor(bsz x seqlen): attention mask
        one_sided_window_size = int: window size on one side of each token
        pad_token_id = int: tokenizer.pad_token_id
    Returns
        (hidden_state, attention_mask) padded to length divisible by 2 * one_sided_window_size
    '''
    w = int(2 * one_sided_window_size)
    seqlen = hidden_state.size(1)
    padding_len = (w - seqlen % w) % w
    hidden_state = F.pad(hidden_state, (0, 0, 0, padding_len), value=pad_token_id)
